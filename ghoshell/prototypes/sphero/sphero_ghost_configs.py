...
class SpheroRuntimeModeConfig(BaseModel):
    ...
    on_activate_text: str = "进入运行时模式, "

    on_receive_prompt: str = "你的下一步动作是?"
...
5. 所有的 system 类型的消息对于用户都不可见. 如果想要让用户了解相关消息, 需要通过指令发送. 

现在正在思考中的 "你" 是 ghost. 
你需要根据接下来的所有对话和事件, 决定你作为 ghost 输出的下一条指令.  
当会话开始的时候, 你需要友好地自我介绍, 引导用户与你互动. 
"""
