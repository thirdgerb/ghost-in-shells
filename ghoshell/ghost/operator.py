...
class Operator(metaclass=ABCMeta):
    ...
    def prepose(self, ctx: "Context") -> Optional[List["Operator"]]:
        """
        每个 Operator 在运行时可能会有前置节点.
        可以将前置节点返回.
        注意!! 符合人类直觉, 按 index 描述先后顺序.
        """
        pass
...
class OperationKernel(metaclass=ABCMeta):
    ...
    def run_dominos(self, ctx: "Context", initial_op: "Operator") -> None:
        ...
        # 队尾进出.
        queue: List[Operator] = [initial_op]
        try:
            count = 0
            while len(queue) > 0:
                op = queue.pop()
                ...
                # 检查是否要插入队列.
                inserts = op.prepose(ctx)
                if inserts is not None and len(inserts) > 0:
                    queue.append(op)
                    queue.append(*reversed(inserts))
                    continue
                ...
                if after is op:
                    ...
                    raise RuntimeException(message="operator loop from %s to %s" % (op, after))
...
class OperationManager(metaclass=ABCMeta):
    ...
    def intend_to(self, uml: UML, params: Dict | None = None) -> "Operator":
        ...
        为什么会有两个参数呢?
        一个是 UML.args, 一个是 intention params:
        - UML.args : 系统内部一个任务进入另一个任务的参数, 相当于 class.__init__()  传入的参数或者配置.
    ...
    def yield_to(self, target: "UML", pid: Optional[str] = None) -> "Operator":
        """
        依赖一个异步任务. 异步任务会发送给一个子进程去执行.
        子进程拥有独立的进程隔离级别, 任务完成后会将结果用 Tasked 消息回调给父进程.
        父进程会根据 tasked 的结果重置目标 task 的状态, 然后正常发起后续流程.
        tasked 传递的状态不能是运行态, 只能是某个中断态 或者 完成态.
        """
        pass
